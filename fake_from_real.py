...
import cv2
import json
from src.preprocess import data2pic,create_folder
import random
import collections
import glob
import tqdm
...
class PulseCreator:
    ...
    def create_fake_pulse(self, time_array, dm, period, width, signal_rate, start_freq, end_freq, fil_path, count):
        ...
        # })
        update = {
            "source_name": "fake_{:.1f}_{:.2f}_{:.2f}_{:.1f}_{:.2f}_{}_{}".format(dm, time_array[0], width, signal_rate,
                                                                               period, self.fake_count, count),
...
def run(creator, fil_path, png_path, fake_anns, count):
    pulse_para = creator.get_random_para()
    # print(pulse_para)
    fake_data, name = creator.create_fake_pulse(*pulse_para, fil_path, count)
    fake_anns.update(
        {
...
def parse_args():
    ...
    parser.add_argument('--is_training', type=int, default=1, help='Whether training or testing. Default: 1')
    parser.add_argument('--out_name', type=str, default='fake_pulsar_test_2', help='output folder')
    parser.add_argument('--nopulse_files', type=str, default='./nopulse_fils/*.fil', help='no pulse fils')
...
def main():
    ...
    nopulse_files = glob.glob(args.nopulse_files)
    if args.is_training:
        ...
        for f in tqdm.tqdm(nopulse_files):
            ...
            for i in range(20):
                pool.apply_async(run, args=(creator, fil_path, png_path, fake_anns, count))
        ...
        for f in tqdm.tqdm(nopulse_files):
            ...
            creator = PulseCreator(fil)
            for i in range(2):
                run(creator, fil_path, png_path, fake_anns, count)
                count += 1
